---
name: extract-paper-knowledge
description: >
  当用户需要从 05_Papers/articles/ 的论文中提取通用知识到 01-04 知识库时触发。
  典型场景：
  - "从这篇论文里提取通用知识"
  - "把论文的研究背景/相关工作整理到 02_AI"
  - "批量补充 01-04 的基础概念"
  - "根据论文的理论部分创建概念笔记"
  - "把 05_Papers 的论文内容沉淀到通用知识库"
  处理流程：识别论文的「研究背景 / 相关工作 / 理论基础」章节 → 提取通用概念与方法 →
  判断所属领域（01_Fundamentals / 02_AI / 03_Robotics / 04_Embodied-AI）→
  检查目标领域是否已有相同主题笔记 → 有则分析差异并追加，无则创建。
  注意：若内容同时属于 02_AI 与 04_Embodied-AI，优先归入 02_AI（通用 AI）。
compatibility: 需要 WebSearch / WebFetch MCP 用于补充信息；需要文件读写工具。
---
